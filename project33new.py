...
# client = Client("cc9feab03d1264ed67c07738cdd42502dd80a8b67fedaf2e5f9b6e9c55a2faad", "e30d3db72358639f29b6280bf1c54fd564e7b1eb5cb13f020739fd197f396e1b") #sudhanshu 
...
def market_order(instrument,side,quantity1):
    ...
    try:
        ...
        if side=='buy':
            ...
            #     quantity=quantity1)
            logger.info(f'ordered a buy on {instrument} with quantity {quantity1}')
        ...
        if side=='sell':
            ...
            #     quantity=quantity1)
            logger.info(f'ordered a sell on {instrument} with quantity {quantity1}')
        print('ordered')
    except Exception as e:
...
def squareoff(instrument,side,quantity1):
    ...
    try:
        ...
        if side=='buy':
            ...
            #     type=Client.ORDER_TYPE_MARKET,
            #     quantity=quantity1)
            logger.info(f'squared off a sell on {instrument} with quantity {quantity1}')
        ...
        if side=='sell':
            ...
            #     quantity=quantity1)
            logger.info(f'squared off a buy on {instrument} with quantity {quantity1}')
...
# %%
def main():
    global df2,tickers,df,ltp,orders,j,short_for_candle,info,orders
    ...
    if len(orders)!=0:
        while True:
            ...
            for i in range(len(orders)):
                ...
                try:
                    df=candle(str(tickers[0]),str(df2['time_frame'][0]))
                    ltp=ltp_price(orders[i]['symbol'])
                    print(ltp)
                    signal=squareoff(orders[i],orders[i]['type'],i)

                    if signal=='squareoffbuy':
                        squareoff(orders[i]['symbol'],'buy',orders[i]['quantity'])
                        orders.pop(i)
                        break

                    if signal=='squareoffsell':
                        squareoff(orders[i]['symbol'],'sell',orders[i]['quantity'])
                        orders.pop(i)
                        break
